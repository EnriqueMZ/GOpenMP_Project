...
var parallelClauses = []string{"if", "num_threads", "default", "shared", "private", "firstprivate", "copyin", "reduction"}
var parallForClauses = []string{"if", "num_threads", "default", "shared", "private", "firstprivate", "copyin", "reduction", "lastprivate", "schedule", "collapse", "ordered"}
var forClauses = []string{"private", "firstprivate", "lastprivate", "reduction", "schedule", "collapse", "ordered", "nowait"}
var thPrivateClauses = []string{""}
var reductionOperators = []string{"+", "*", "-", "&", "|", "^", "&&", "||"}
...
/*func clauseContentNum(body string) (int, []string) { // La necesitaremos más adelante.
	contAll, res := splitFirstBySep(body, ")")
	cont, err := strconv.Atoi(strings.Trim(contAll, " "))
	if err != nil {
		panic("Valor no válido en num_threads")
	}
	return cont, res
}*/
...
func clauseContentDef(body string) (Default_Type, []string) {
	...
	fmt.Println("Contenido sin espacios ni comas:", cont)
	switch cont {
	case "shared":
		typ = 0
	case "none":
...
func ProcessPragma(pragma string) Pragma {
	fmt.Println("Procesando nuevo pragma: \"", pragma, "\"\n")
	var PragmaProcesed Pragma
	var oneDefault, oneIf, oneNThreads, oneThPrivate bool = false, false, false, false
	var prgTyp Pragma_Type
	var defTyp Default_Type
	var clauseType Clause_Type
	var clause, ifCont, nThreadsCont string
	var clauseList, contentList, variableList, privateList, sharedList, firstList []string
	var reductionList []Reduction_Type
	var operator Red_Operator
	cond, body := isPragmaGomp(pragma)
	if cond {
		fmt.Println("Es pragma gomp:", body, "\n") // eliminar
	} else {
	...
	case 0:
		...
		PragmaProcesed.Def_Num_threads = Gomp_get_num_routines()
	...
	fmt.Println("Tipo de pragma:", prgTyp)                 // eliminar
	fmt.Println("Clausulas del pragma:", clauseList, "\n") // eliminar

	for {
		if len(clauseList) == 0 || clauseList[0] == "" { // Si no hay clausulas, o ya se ha procesado el string de clausulas.
			fmt.Println("Fin del procesado del pragma. \n")
			break
		}
		clauseType, clause, clauseList = splitClauses(prgTyp, clauseList[0])
		fmt.Println("Tipo de clausula:", clause)
		switch clauseType {
		case 0:
			if oneDefault == true {
			...
			fmt.Println("Tipo de default:", PragmaProcesed.Default)
			fmt.Println("Resto", clauseList, "\n")
		case 1:
			if oneIf == true {
			...
			fmt.Println("Contenido del if:", PragmaProcesed.If_content)
			fmt.Println("Resto", clauseList, "\n")
		case 2:
			if oneNThreads == true {
			...
			fmt.Println("Valor del campo:", nThreadsCont)
			fmt.Println("Resto", clauseList, "\n")
		case 3:
			...
			fmt.Println("Variables:", PragmaProcesed.Variable_List)
			fmt.Println("Variables shared:", PragmaProcesed.Shared_List)
			fmt.Println("Resto", clauseList, "\n")
		case 4:
			contentList, clauseList = clauseContentList(clauseList[0])
			res, rep := repeat(contentList, PragmaProcesed.Variable_List)
			...
			fmt.Println("Variables:", PragmaProcesed.Variable_List)
			fmt.Println("Variables private:", PragmaProcesed.Private_List)
			fmt.Println("Resto", clauseList, "\n")
		case 5:
			contentList, clauseList = clauseContentList(clauseList[0])
			res, rep := repeat(contentList, PragmaProcesed.Variable_List)
			...
			fmt.Println("Variables:", PragmaProcesed.Variable_List)
			fmt.Println("Variables firstprivate:", PragmaProcesed.First_List)
			fmt.Println("Resto", clauseList, "\n")
		case 6:
		...
		case 7:
			...
			fmt.Println("Variables:", PragmaProcesed.Variable_List)
			fmt.Println("Variables reduction:", PragmaProcesed.Reduction_List)
			fmt.Println("Resto", clauseList, "\n")
		...
		case 13:
			...
			PragmaProcesed.Variable_List = concat(contentList, PragmaProcesed.Variable_List)
			fmt.Println("Variables:", PragmaProcesed.Variable_List)
			fmt.Println("Variables Threadprivate:", PragmaProcesed.First_List)
			fmt.Println("Resto", clauseList, "\n")
		default:
			panic("En proceso...")
		}
...
var pragma_threadprivate string = "  //pragma  gomp threadprivate   (a,b) " // EJEMPLO THREADPRIVATE
var pragma_parallel string = "  //pragma   gomp   parallel  if (n>10) , num_threads(n + 5) private(a, b,c )   default(shared), shared(d)  private(A,e) , firstprivate(B) reduction(+:f,g) , reduction(-: h, i) " // EJEMPLO PARALLEL

func main() {
	fmt.Println("Inicio\n")
	ProcessPragma(pragma_threadprivate)
	ProcessPragma(pragma_parallel)
	fmt.Println("Fin\n")
}
