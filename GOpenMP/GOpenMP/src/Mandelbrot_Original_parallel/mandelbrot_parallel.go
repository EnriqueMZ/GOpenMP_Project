...
var (
	...
	height	= flag.Int("h", 16384, "height of the output image in pixels")
	width	= flag.Int("w", 16384, "width of the output image in pixels")
)
...
func fillPixel(m *img, i, j int) {	// normalized from -2.5 to 1 estoy toqueteando comentarios
	xi := 3.5*float64(i)/float64(m.w) - 2.5
	// normalized from -1 to 1 estoy toqueteando comentarios
