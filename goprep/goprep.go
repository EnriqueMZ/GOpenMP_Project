...
func PipeInit(iReader io.Reader) *Pipe {
	...
	go func() {
		...
		for tok != token.EOF {
			...
			if tok == token.COMMENT && !(isPragmaGomp(str)) { // MODIFICATE
...
// Check if a string is a "pragma gomp"
func isPragmaGomp(str string) bool {
	res := strings.HasPrefix(noSpaces(str), "//pragmagomp")
	return res
}
